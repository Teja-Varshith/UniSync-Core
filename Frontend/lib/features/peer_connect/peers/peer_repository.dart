...
class PeerRepository {
  ...
  // ── Like a peer card ───────────────────────────────────────────────────────
  // Uses arrayUnion — Firestore ignores duplicates automatically
  Future<void> likeCard(String cardOwnerId, String likerId) async {
    try {
      await _peer.doc(cardOwnerId).update({
        'likedBy': FieldValue.arrayUnion([likerId]),
      });
    } catch (e) {
      print('Error liking card: $e');
      rethrow;
    }
  }
 
  // ── Unlike a peer card ─────────────────────────────────────────────────────
  Future<void> unlikeCard(String cardOwnerId, String likerId) async {
    try {
      await _peer.doc(cardOwnerId).update({
        'likedBy': FieldValue.arrayRemove([likerId]),
      });
    } catch (e) {
      print('Error unliking card: $e');
      rethrow;
    }
  }
 
  // ── Check if a user has liked a card ──────────────────────────────────────
  Future<bool> hasLiked(String cardOwnerId, String likerId) async {
    try {
      final doc = await _peer.doc(cardOwnerId).get();
      if (!doc.exists) return false;
      final data = doc.data() as Map<String, dynamic>;
      final likedBy = List<String>.from(data['likedBy'] ?? []);
      return likedBy.contains(likerId);
    } catch (e) {
      return false;
    }
  }
